...
- (void)decrementMoves
{
    self.movesLeft--;
    [self updateLabels];
    NSString* nextLevel = self.level.nextLevel;
    NSLog(@"NExt level file is: %@",  self.level.nextLevel);
    
    if (self.score >= self.level.targetScore)
    {
        self.gameOverPanel.image = [UIImage imageNamed:@"LevelComplete"];
        [self.scene removeAllMaskSprites];
        self.level = [[RWTLevel alloc] initWithFile:nextLevel];
        self.scene.level = self.level;
        [self.scene  addTiles];
